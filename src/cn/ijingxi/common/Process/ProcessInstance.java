...
{	
	...
	//静态变量与构造函数
	//
	static jxStateMachine<InstanceState,InstanceEvent,CallParam> NodeSM=null;
	static jxStateMachine<InstanceState,InstanceEvent,CallParam> ProcessSM=null;
	static
	{
		NodeSM=new jxStateMachine<InstanceState,InstanceEvent,CallParam>();
		ProcessSM=new jxStateMachine<InstanceState,InstanceEvent,CallParam>();
		//初始化节点的状态转换
		...
		//NodeSM.AddTrans(InstanceState.Doing, InstanceEvent.Cancel, InstanceState.Canceled, new NodeCancel());
    ...
    public void Pause(IExecutor Caller,String msg) throws Exception
    {
		CallParam param=new CallParam(null, Caller, msg);
		param.addParam(this);
		State=ProcessInstance.ProcessSM.Happen(State, InstanceEvent.Pause, param);
    }
    public void ReDo(IExecutor Caller,String msg) throws Exception
    {
		CallParam param=new CallParam(null, Caller, msg);
		param.addParam(this);
		State=ProcessInstance.ProcessSM.Happen(State, InstanceEvent.Trigger, param);
    }
    public void Cancle(IExecutor Caller,String msg) throws Exception
    {
		CallParam param=new CallParam(null, Caller, msg);
		param.addParam(this);
		State=ProcessInstance.ProcessSM.Happen(State, InstanceEvent.Cancel, param);
    }
    public void Close(IExecutor Caller,String msg) throws Exception
    {
		CallParam param=new CallParam(null, Caller, msg);
		param.addParam(this);
		State=ProcessInstance.ProcessSM.Happen(State, InstanceEvent.Close, param);
    }
    ...
    public void Node_Pause(String NodeName,IExecutor Caller,String msg) throws Exception
    {
		...
		param.addParam(node);
		node.setState(ProcessInstance.NodeSM.Happen(node.getState(), InstanceEvent.Pause, param));
    }
