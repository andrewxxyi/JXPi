...
import cn.ijingxi.stub.dal.*;
import cn.ijingxi.stub.general.IDo;
import cn.ijingxi.stub.general.jxHttpClient;
 ...
 * 使用方法：
 * 1、初始化
 * cn.ijingxi.stub.session s=new cn.ijingxi.stub.session("192.0.54.123",10008,null);
 * 2、使用
...
public class session {
    ...
    //
    //列表分配给某人的任务
    public Task task_GET(String taskid) {
        try {
            //访问准备
            clear();
            //client.addQueryString("TaskID", taskid);

            //发起REST访问
            jxJson rs = client.get("/task/");
            if (rs != null)
                //获取结果
                return new Task(rs);
        } catch (Exception e) {
    ...
    //列表分配给某人的任务
    public Queue<Task> task_listMyTask(String peopleid,String state) {
        try {
            Queue<Task> result=new LinkedList<>();
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            json.setSubObjectValue("PeopleID", peopleid);
            json.setSubObjectValue("State", state);

            //访问准备
            clear();

            //发起REST访问
            jxJson rs = client.post("/task/listMyTask", json);
            if (rs != null) {
                for (jxJson sub : rs) {
                    result.offer(new Task(sub));
                }
                //获取结果
                return result;
            }
    ...
    //列表本组的所有任务
    public Queue<Task> task_listTeamTask(String teamID,String state) {
        try {
            Queue<Task> result=new LinkedList<>();
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            ...
            //发起REST访问
            jxJson rs = client.post("/task/listTeamTask", json);
            if (rs != null) {
                for (jxJson sub : rs) {
                    result.offer(new Task(sub));
                }
    ...
    //灭除问题
    public boolean task_PUT(String taskID,String content) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            ...
            //发起REST访问
            jxJson rs = client.put("/task",json);
            if (rs != null)
    ...
    //创建任务
    public boolean task_POST(String name,String peopleid,String startat) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            json.setSubObjectValue("Name", name);
            json.setSubObjectValue("PeopleID", peopleid);
            json.setSubObjectValue("StartTime", startat);
            ...
            //发起REST访问
            jxJson rs = client.post("/task",json);
            if (rs != null)
                return jxJson.checkResult(rs);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    //
    //
    public void andriodTest_andriodClick_SW1(boolean close) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            json.setSubObjectValue("Action", "andriodTest");
            json.setSubObjectValue("Func", "andriodClick_SW1");
            json.setSubObjectValue("State", close?"close":"open");

            //访问准备
            clear();

            //发起REST访问
            jxJson rs= client.post("/jxLua/call", json);
        } catch (Exception e) {
            e.printStackTrace();
        }
        //return null;
    }
    public void andriodTest_andriodClick_SW12(boolean close) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            json.setSubObjectValue("Action", "andriodTest");
            json.setSubObjectValue("Func", "andriodClick_SW2");
            json.setSubObjectValue("State", close?"close":"open");
    ...
    public boolean andriodTest_andriodGetState(boolean close) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            json.setSubObjectValue("Action", "andriodTest");
            json.setSubObjectValue("Func", "andriodGetState");
    ...
    public jxJson jxLua_call(jxJson param) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
            //json.setSubObjectValue("PeopleID", peopleid);
            ...
            //发起REST访问
            return client.post("/jxLua/call", json);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    ...
    //
    //获取用户所属项目组
    public jxJson jxLua_call(String Action, String Func, jxJson param) {
        try {
            //参数准备
            jxJson json = jxJson.GetObjectNode("r");
			if(param!=null)
				for(jxJson sub:param)
            		json.setSubObjectValue(sub.getName(), sub.getValue());
