...
public class jxJson implements Iterable<jxJson>
{
    ...
    public Object GetSubValue(String SubName) throws Exception {
        utils.checkAssert( SubName != null , "需要给出子对象的名字");
        utils.checkAssert( Type == NodeType.Object, "只有对象才可能直接读取子节点的值");
    ...
    public String GetSubValue_String(String SubName) throws Exception {
        try {
            Object obj = GetSubValue(SubName);
    ...
    public int GetSubValue_Integer(String SubName) throws Exception {
        try {
            Object obj = GetSubValue(SubName);
    ...
    public float GetSubValue_Float(String SubName) throws Exception {
        try {
            Object obj = GetSubValue(SubName);
            //jxLog.logger.debug(obj.getClass().toString());
            if (obj != null)
    ...
    public double GetSubValue_Double(String SubName) throws Exception {
        try {
            Object obj = GetSubValue(SubName);
    ...
    public boolean GetSubValue_Boolean(String SubName) throws Exception {
        try {
            Object obj = GetSubValue(SubName);
    ...
    public UUID GetSubValue_UUID(String SubName) throws Exception {
        try {
            Object obj = GetSubValue(SubName);
            if (obj != null && (obj instanceof String))
    ...
    public jxJson GetSubObject(String SubName) throws Exception {
        utils.checkAssert(SubName != null, "需要给出子对象的名字");
        if(Type==NodeType.Undefined)return null;
        utils.checkAssert(Type == NodeType.Object, "只有对象才可能直接读取子节点的值");
        if (SubObjectList == null)
            return null;
        jxJson node = SubObjectList.search(SubName);
    ...
    {
        utils.checkAssert( Type==NodeType.Array, "只有数组才能读取子元素");
        return Array;
    }
    ...
    public void AddSubObjNode(jxJson sub) throws Exception {
        utils.checkAssert(sub != null, "不能添加空的子对象");
        if (Type == NodeType.Undefined) {
            Type = NodeType.Object;
            SubObjectList = new jxLink<>();
        } else
            utils.checkAssert(Type == NodeType.Object, "只有对象节点才能添加子对象");
        utils.checkAssert(!SubObjectList.Exist(sub.Name), "节点已存在：" + sub.Name);
        SubObjectList.addByRise(sub.Name, sub);
    }
    public void UpdateSubObjNode(jxJson sub) throws Exception {
        utils.checkAssert(sub != null, "不能添加空的子对象");
        if (Type == NodeType.Undefined) {
            Type = NodeType.Object;
            SubObjectList = new jxLink<>();
    ...
    {
        utils.checkAssert( el!=null, "不能添加空的子元素");
        utils.checkAssert( Type==NodeType.Array, "只有数组节点才能添加子元素");
        synchronized (Array)
        {
    ...
    public String TransToString()
    {
        ...
        {
            //String s=TransValueToJSONString(Value);
            //jxLog.logger.debug(String.format("key:%s,valueB:%s,valueE:%s",Name,Value,s));
    ...
    public static jxJson JsonToObject(String json) {
        ...
        if (nodetype == NodeType.Array) {
            ...
            while (m.find()) {
                String v = m.group(0);
                //utils.P("value:"+v);
                jxJson arrsub = JsonToObject(v);
                arrsub.ArrayIndex=newNode.Array.size();
                newNode.Array.add(arrsub);
            }
            ...
            //jxLog.logger.debug(jxJson);
            newNode.Value = Trans.transStringToObjectWithType(jxJson.replaceAll("\\r\\n", "\r\n"));
            //jxLog.logger.debug(newNode.Value );
        }
        return newNode;
    }
